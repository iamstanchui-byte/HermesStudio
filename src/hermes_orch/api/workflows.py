...
# ===== v2.1 (2026-07-30): Parameter hints =====
# POST /api/workflows/{id}/suggest-vars
# Walks the workflow's step_template, finds every `{{var}}`
# placeholder in any params_template, and returns a list of
# suggested variable definitions:
#   [{name, type, default, description, used_in: [step_names], examples}, ...]
#
# This is a LOCAL extraction (no LLM call). The user's request
# for "LLM suggest var types" is partially served: we suggest
# types based on the LITERAL VALUES that appear in the step's
# params_template alongside the placeholder. For example, a step
# with params {"symbols": "{{tickers}}", "interval": "1d"} and
# another with params {"tickers": "AAPL,GOOG"} would get:
#   {name: "tickers", type: "string", default: "AAPL,GOOG",
#    description: "Used in step X", examples: ["AAPL,GOOG"]}
#
# A future v2.2 can layer the LLM on top (use the step's action
# text to suggest better names + descriptions). For v2.1, this
# local approach is fast, deterministic, and testable without
# requiring the LLM provider to be configured.
def _extract_placeholders_in_value(v, out):
    """Recursively walk a value (dict / list / string) and collect
    every `{{var}}` placeholder name. Same logic as
    visual_workflow.js::_collectPlaceholdersInValue but in Python."""
    if v is None:
        return
    if isinstance(v, str):
        import re
        ...
        for item in v:
            _extract_placeholders_in_value(item, out)
    elif isinstance(v, dict):
        for val in v.values():
            _extract_placeholders_in_value(val, out)


def _infer_type_from_literal(literal):
    """Heuristic type inference for a literal value found in a
    step's params_template. Returns one of: 'string', 'int',
    'float', 'bool', 'list[string]'.
    Conservative: when ambiguous, returns 'string' (the safe
    default that operators can refine in the UI)."""
...
def _collect_literal_values_for_placeholder(step, var_name):
    """For a given {{var_name}} placeholder, find LITERAL values
    in the same step's params_template that could be the default.
    "Same step" because defaults typically come from the step that
    USES the variable, not the step that DEFINES it."""
    out = []
    params = step.get("params_template") or {}
    ...
    for k, v in params.items():
        if isinstance(v, str) and ("{{" + var_name + "}}") in v:
            # This key's value contains the placeholder; look at
            # OTHER keys in the same params for potential default.
            for k2, v2 in params.items():
                if k2 == k:
                    continue
                if isinstance(v2, (str, int, float, bool)):
                    out.append(v2)
                elif isinstance(v2, list) and v2 and all(isinstance(x, (str, int, float, bool)) for x in v2):
                    out.append(v2)
    return out


@router.post("/{workflow_id}/suggest-vars")
async def suggest_vars(workflow_id: str, request: Request) -> dict:
    """Return parameter suggestions for the workflow's
    step_template. Local extraction (no LLM call); see the
    docstring on `suggest_vars` for the algorithm + future
    LLM-backed extension."""
    db = request.app.state.db
    row = await db.fetchone(
        "SELECT id, step_template, variables FROM workflow_packages WHERE id = ?",
        (workflow_id,),
    )
    if not row:
        raise HTTPException(404, f"Workflow not found: {workflow_id}")
    try:
        step_template = json.loads(row.get("step_template") or "[]")
    except (json.JSONDecodeError, TypeError):
        step_template = []
    if not isinstance(step_template, list):
        step_template = []
    # Collect every placeholder across all steps + the steps
    # each one is used in (for the description / used_in list).
    placeholder_to_steps: dict[str, list[str]] = {}
    for step in step_template:
        if not isinstance(step, dict):
            continue
        params = step.get("params_template") or {}
        placeholders = set()
        _extract_placeholders_in_value(params, placeholders)
        for ph in placeholders:
            placeholder_to_steps.setdefault(ph, []).append(step.get("name") or "?")
    # Existing variable names — we still return suggestions for
    # them (the UI can show "already defined"), but mark them so
    # the user doesn't double-add by accident.
    try:
        existing_vars = json.loads(row.get("variables") or "[]")
    except (json.JSONDecodeError, TypeError):
        existing_vars = []
    existing_names = set(
        v.get("name") for v in existing_vars if isinstance(v, dict) and v.get("name")
    )
    # Build suggestions
    suggestions = []
    for ph, step_names in sorted(placeholder_to_steps.items()):
        # Find a default value from the FIRST step that uses the
        # placeholder. We don't try to be clever about cross-step
        # reconciliation — the user can adjust the type + default
        # in the UI.
        default_value = None
        ...
        for step in step_template:
            if not isinstance(step, dict):
                continue
            if (step.get("name") or "?") not in step_names:
                continue
            literals = _collect_literal_values_for_placeholder(step, ph)
            if literals:
                default_value = literals[0]
                inferred_type = _infer_type_from_literal(literals[0])
                break
        # Description: "Used in N step(s): a, b, c" (truncate to
        # 3 names + "..." if there are more).
        n = len(step_names)
        if n == 1:
            used_in_desc = "Used in step '" + step_names[0] + "'"
        elif n <= 3:
            used_in_desc = "Used in " + str(n) + " steps: " + ", ".join(step_names)
        else:
            used_in_desc = "Used in " + str(n) + " steps: " + ", ".join(step_names[:3]) + ", ..."
        suggestions.append({
            "name": ph,
            "type": inferred_type,
            "default": default_value if default_value is not None else "",
            "description": used_in_desc,
            "used_in": step_names,
            "already_defined": ph in existing_names,
        })
    return {
