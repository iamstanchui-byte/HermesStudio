...
class MarkTemplate(BaseModel):
    ...
    # Plan B (2026-07-23): if True (default), also LLM-synthesize
    # the project's plan.md so it's a clean reusable doc, not the
    # original L3 trace dump. Set False to keep the existing plan.md.
...
async def mark_project_as_template(
    ...
    If `regenerate` is True (default), also LLM-synthesize a clean
    plan.md using the Hermes 4-layer framework (Plan B). The
    synthesized doc drops L2 specific values and L3 trace
    scaffolding, so a future run of the template starts from a
    clean reusable doc, not a trace dump.
    """
    ...
    plan_regen_error = None
    if body.regenerate:
        try:
            cfg = request.app.state.config
            pdir = _project_dir(request, project_id)
            evidence = await _gather_plan_evidence(db, pdir, project_id, proj)
            llm_cfg = cfg.get("llm", {})
            new_plan = await _call_llm_for_plan_synthesis(
                evidence, llm_cfg, project_id
            )
            ok, err = _validate_plan_md(new_plan)
            if not ok:
                plan_regen_error = f"validation failed: {err}"
            else:
                pdir.mkdir(parents=True, exist_ok=True)
                (pdir / "plan.md").write_text(new_plan, encoding="utf-8")
                plan_regenerated = True
                await audit_log(
                    db, "project.plan_regenerated",
                    actor="llm", project_id=project_id,
                    payload={"source": "mark-template",
                             "plan_size_bytes": len(new_plan.encode("utf-8"))},
                )
        except HTTPException as e:
            plan_regen_error = f"HTTP {e.status_code}: {e.detail}"
        except Exception as e:
            plan_regen_error = f"{type(e).__name__}: {e}"
    ...
    return {
        ...
        "plan_regen_error": plan_regen_error,
    }
...
# ===== Plan B: LLM-synthesize plan.md when mark-template (2026-07-23) =====
#
# Same Hermes 4-layer framework as promote-to-skill above, but for
# plan.md instead of SKILL.md. The plan.md is the procedural doc the
# agent reads BEFORE doing each task. Without LLM synthesis, the
# current plan.md is whatever the planner LLM produced at project
# create time — full of L3 noise (plan_generated metadata, internal
# action names, failed attempts). When the project is used as a
# TEMPLATE (i.e. cloned for a schedule), we want a clean reusable
# doc, not the trace dump.
#
# The flow:
#   POST /api/schedules/project/{id}/mark-template body={regenerate: true}
#     1. Set is_template=1
#     2. Read existing plan.md (if any) as evidence
#     3. Call LLM with 4-layer prompt
#     4. Validate output (no L3 scaffolding, no specific L2 values)
#     5. Overwrite plan.md with the LLM result
#     6. Audit project.plan_regenerated

# Cap the synthesized plan.md to keep it sane
_PLAN_MAX_FILE_BYTES = 60_000

# L3 markers that must NOT appear in the synthesized plan.md
_L3_BAD_TOKENS = (
    "[cite:",
    "task.completed@",
    "DECISION: PASS",
    ...
    "iteration_completed@",
    "_iteration_review:",
    # L3 planner scaffold that gets auto-generated by the planner
    "## Plan History",
    "## Task Results",
    "## Coord Verdicts",
)
...
_LLM_PLAN_SYNTHESIS_PROMPT = """You are converting a project's plan.md into a clean, reusable TEMPLATE plan.md following the Hermes 4-layer separation framework.

# Goal
Produce a plan.md that, when an agent reads it before each task, makes the agent UNDERSTAND the template's purpose and execute the procedure — not just glance at it and start. The original plan.md is full of trace dump (plan_generated metadata, L3 task IDs, internal action names, failed attempts). Strip all of that.

# 4-Layer Separation (mandatory)
For every piece of evidence below, classify it:
- **L0 structure** (step count, ordering, dependencies) — KEEP, rewrite as `## Step N: <verb> <thing>`
- **L1 actions** (which tool / API / website / command was used) — KEEP, rewrite as concrete tool calls
- **L2 data** (specific values: coordinates, dates, folder IDs, raw JSON, names) — DROP. Replace with `{{var}}` placeholders.
- **L3 decisions** (plan_generated metadata, [cite:task.completed@…] tokens, PASS/FAIL verdicts, coord_pickup handoffs, iteration_completed markers) — DROP entirely. Internal to the originating engine.

# Cardinal test
Does this line tell the agent WHAT TO DO at template-run time? If yes (L0/L1) → keep. If it describes what already happened on the previous run or whether it was approved (L2/L3) → drop.

# Output schema (strict, follow exactly)
- File starts with `---` (byte 0) → YAML frontmatter.
- Required frontmatter fields:
  - `project_id`: the requested project_id below.
  - `name`: kebab-case, MUST match the project name.
  - `description`: ≤ 200 chars. Trigger-class (what kind of user request should invoke this template), NOT the project goal literally.
  - `version`: 0.1.0.
  - `state`: "ready".
  - `tasks`: list of step summaries, each with {name, agent_role, depends_on, action}.
  - `template`: 1 (this is a template).
- Body sections, in order: `## Overview`, `## When to Use`, `## Steps` (with `## Step N: <verb> <thing>` sub-headings), `## Variables` (the {{var}} placeholders + descriptions), `## Verification`.
- Total file size: target 4-10 KB, hard cap 60 KB.

# Step format (mandatory)
Each `## Step N: <verb> <thing>` must specify:
1. **Action verb** (Fetch, Search, Read, Compose, Verify, etc.) — strong verb, not a noun.
2. **Tool** to use (web_search, web_extract, terminal, read_file, patch, web_fetch, hermes skill, etc.). NEVER use internal engine names like `coord_pickup`, `_iteration_review:1`, `handoff: coord_pickup`.
3. **Target**: an exact URL, exact CLI command, or exact search query. If it depends on a {{var}}, write the var name (e.g. `web_search query: "{{user_topic}}"`).
4. **Completion criterion**: a checkable assertion (e.g. "result contains forecast for next 7 days with high/low °C and precipitation probability").
5. NO fetched data values, task IDs, or PASS/FAIL markers inside the step.

# Variables section (mandatory)
List every `{{var}}` placeholder used in the steps, one per line:
- `{{var_name}}` (type) — description, ≤ 80 chars. required: true/false.

# Anti-patterns to drop (these WILL fail validation)
- `[cite:task.completed@...]` (L3 trace citation)
- `## Plan History` / `## Task Results` / `## Coord Verdicts` / `## Files (artifacts)` headings (L3)
- Internal engine names: `coord_pickup`, `_iteration_review:1`, `handoff: coord_pickup`
- Specific values: actual coordinates, specific dates, raw JSON dumps, today's date, exact file names
- `DECISION: PASS` / `DECISION: FAIL` markers (L3)
- `_iteration_review:1` style action names (L3 coord scaffolding — omit the step entirely)
- `plan_generated@...` (L3 planner metadata — DROP)

# Evidence
{evidence}

# Output
Output ONLY the plan.md content, starting with `---` (the frontmatter opening). No preamble, no explanation, no code fence wrappers. End with exactly one trailing newline.
"""


async def _gather_plan_evidence(db, pdir, project_id: str, proj: dict) -> str:
    """Build the evidence block for plan.md LLM synthesis.

    Different from skill evidence: we need less detail (plan.md is
    shorter than SKILL.md). We include:
    - project metadata (name, goal, state)
    - task list (L0 structure + L1 actions only, no L3 noise)
    - the CURRENT plan.md body (so the LLM can see what to clean up)
    """
    parts: list[str] = ["# Project evidence\n"]
    parts.append("## Project metadata\n")
    parts.append(f"- id: {project_id}\n")
    ...
    # Tasks (L0+L1 only)
    tasks = await db.fetchall(
        "SELECT name, agent_role, action, status, depends_on "
        "FROM tasks WHERE project_id = ? "
        "ORDER BY created_at ASC",
        (project_id,),
    )
    ...
    if completed:
        parts.append("\n## Tasks (L0 structure + L1 actions)\n")
        for t in completed:
            name = t.get("name") or "task"
            role = t.get("agent_role") or "?"
            short_act = (t.get("action") or "?").split(":")[-1][:50]  # strip L3 prefix
            deps = t.get("depends_on") or []
            deps_str = f" (after: {', '.join(deps)})" if deps else ""
            parts.append(f"- **{name}** [{role}]{deps_str}: action=`{short_act}`\n")

    # Current plan.md body (the LLM needs to see what to clean up)
    plan_path = pdir / "plan.md"
    if plan_path.exists():
        plan_text = plan_path.read_text(encoding="utf-8", errors="replace")
        # Strip the frontmatter so the LLM only sees the body (less noise)
        if plan_text.startswith("---"):
            fm_end = plan_text.find("\n---\n", 3)
            if fm_end > 0:
                plan_body = plan_text[fm_end + 5:]
            else:
                plan_body = plan_text
        else:
            plan_body = plan_text
        parts.append(
            f"\n## Current plan.md body ({len(plan_text)} bytes -> "
            f"{len(plan_body)} after frontmatter strip)\n```\n"
            f"{plan_body[:3000]}\n```\n"
        )
...
async def _call_llm_for_plan_synthesis(
    evidence: str, llm_cfg: dict, project_id: str
) -> str:
    """Call the LLM to synthesize a clean plan.md. Mirrors the
    promote-to-skill pattern (httpx, MiniMax, <think> strip, fence
    strip, dump on failure)."""
    import httpx
    import sys as _sys
    ...
    api_key = llm_cfg.get("api_key") or ""
    model = llm_cfg.get("model") or "MiniMax-M3"
    # Plan synthesis uses shorter evidence than workflow, but MiniMax
    # load spikes can push calls to 60-180s. Bump default to 240s.
    timeout = float(llm_cfg.get("timeout_seconds") or 240)
    if not api_key:
        raise HTTPException(
            503,
            "LLM api_key not configured — set llm.api_key in config.yaml "
            "before regenerating plan.md.",
        )

    # Use plain str.replace, NOT str.format, because the prompt has
    # literal `{{var}}` examples (which the LLM needs to see
    # verbatim) — str.format would mangle them to `{var}`.
    prompt = _LLM_PLAN_SYNTHESIS_PROMPT.replace("{evidence}", evidence)
        ...
        "messages": [
            {
                "role": "system",
                "content": (
                    "You output plan.md only. Strict 4-layer separation. "
                    "Drop ALL L2 values, ALL L3 scaffolding. "
                    "Use {{var}} placeholders for any value that would "
                    "change on a re-run. "
    ...
    try:
        ...
        raise HTTPException(502, f"LLM response shape unexpected: {e}")
    print(
        f"[plan-synth] LLM response: text_len={len(text) if isinstance(text, str) else 'N/A'}, "
        f"finish_reason={data['choices'][0].get('finish_reason')}, "
        f"first_100={(text[:100] if isinstance(text, str) else 'N/A')!r}",
        file=_sys.stderr,
    )
...
def _validate_plan_md(content: str) -> tuple[bool, str]:
    """Validate the LLM-produced plan.md against the 4-layer rules.

    Returns (ok, error_message). On failure, the caller should refuse
    to overwrite the existing plan.md and tell the user to re-run or
    hand-fix.
    """
    if not content or not isinstance(content, str):
        return False, "content is empty"
    if not content.startswith("---"):
        return False, "must start with '---' (YAML frontmatter)"
    # Parse frontmatter
    m = re.match(r"^---\n(.*?)\n---\n?(.*)", content, re.DOTALL)
    if not m:
        return False, "frontmatter not properly delimited"
    fm_str, body = m.group(1), m.group(2)
    # Frontmatter must parse as YAML with project_id + name
    import yaml
    try:
        fm = yaml.safe_load(fm_str) or {}
    except Exception as e:
        return False, f"frontmatter YAML parse failed: {e}"
    if not fm.get("project_id"):
        return False, "frontmatter missing required 'project_id'"
    if not fm.get("name"):
        return False, "frontmatter missing required 'name'"
    # Body should have at least ## Steps
    if "## Steps" not in body and "## Step " not in body:
        return False, "body missing required '## Steps' section"
    # Anti-L3 dump guards: scan the full content
    for tok in _L3_BAD_TOKENS:
        if tok in content:
            return False, f"plan.md contains forbidden L3 scaffolding: {tok!r}"
    # Size cap
    if len(content.encode("utf-8")) > _PLAN_MAX_FILE_BYTES:
        return False, f"plan.md too large: > {_PLAN_MAX_FILE_BYTES} bytes"
    return True, ""
