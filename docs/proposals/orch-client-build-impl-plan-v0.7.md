# Orch Client Build — Implementation Plan v0.7 (final plan-only iteration) + cert-pinning patch + v0.7.1 bootstrapper + v0.7.2 firewall auto-add + v0.7.3 real TLS cert fingerprint pinning

**Date:** 2026-08-15 (v0.7.3 patch added)
**Status:** PROPOSAL for review (Perplexity + operator) — v0.7 + cert-pinning patch + v0.7.1 bootstrapper + v0.7.2 firewall auto-add + v0.7.3 real TLS cert fingerprint pinning
...
## 0.quint v0.7.3 real TLS cert fingerprint pinning patch (2026-08-15)

After the v0.7.2 firewall auto-add landed, the orch server's HTTPS
support was enabled on the production Windows A host (2026-08-15, see
operator run-log). The TLS layer is now live: `Uvicorn running on
https://0.0.0.0:8765` with a self-signed cert generated by
`hermes-orch gen-cert` (v3.12.0).

But the **client-side cert fingerprint pinning per v0.7 §1.6 was NOT
yet enforced** in the bootstrapper. The pre-flight `Test-CertFingerprint`
was a FORMAT check only (regex `^[0-9a-f]{64}$` on the user-pasted
64-char hex); the actual `Wait-ForEnrollment` poll used
`Invoke-WebRequest`, which (a) has a PS 5.1 / .NET Framework 4.x SChannel
bug that breaks TLS 1.2 endpoints, and (b) trusts the default OS cert
validation — which would fail outright for the self-signed orch cert
(since it's not in the OS trust store).

**The combined effect**: before v0.7.3, the bootstrapper's enrollment
poll could not actually complete end-to-end against a self-signed orch.
The format check on the user-pasted fingerprint was a hopeful lie.

**What v0.7.3 changes:**

- **`Wait-ForEnrollment` rewritten** to use
  `[System.Net.Http.HttpClient]` with a custom
  `HttpClientHandler.ServerCertificateCustomValidationCallback`. The
  callback:
  1. Computes SHA-256 of `cert.Export([X509ContentType]::Cert)` (the
     cert's DER bytes) — NOT `GetCertHash()`, which returns the cert's
     signature algorithm hash (often SHA-1, not what we want)
  2. Lowercase hex, joined
  3. Compares to the user-pasted `$CertFingerprint` (64-char hex)
  4. Returns `$true` only if match; `$false` (fail closed) on any
     exception in the callback or on mismatch

- **Force TLS 1.2** at the handler init:
  `[Net.ServicePointManager]::SecurityProtocol = Tls12`. PS 5.1's
  default is SSL3 / TLS 1.0, which the orch's cert rejects at the
  handshake stage (before the cert validation callback fires).

- **New `-CertFingerprint` parameter** on `Wait-ForEnrollment`, passed
  from the call site (which already has `$CertFingerprint` in scope
  from the prompt).

- **Improved `CERT_MISMATCH` error message** to cover both cert
  mismatch AND TLS handshake failure (e.g. orch using TLS < 1.2). The
  user sees the same plain-English message either way; the cause is
  the same from the user's perspective (re-verify fingerprint + FQDN +
  ask operator about TLS version).

- **Explicit disposal** of `HttpClient`, `HttpClientHandler`, and
  `HttpRequestMessage` (was missing in v0.7.1 Draft 4 which used
  `Invoke-WebRequest`; `HttpClient` requires explicit `Dispose`).

- **Version bump** v0.7.2 → v0.7.3 in 3 places (header, banner, design
  reference). Internal Draft 5 → Draft 6 (one new draft for the
  real cert pinning).

**What v0.7.3 does NOT change:**

- **Cert fingerprint typing friction** — still addressed by v0.8
  invite token (one short token encodes cert fp + key_id + token).
  v0.7.3 makes the cert pin ACTUALLY ENFORCED, but the user still
  has to type a 64-char hex string (or paste it). The 80% UX win is
  in v0.7.2 (firewall) + v0.7.3 (real cert pin) together.
- **`Test-CertFingerprint` format check** — still kept as a pre-flight
  sanity check. It's now an early-fail for the common typo case
  (user pastes 63 chars, or includes a colon, etc.) before we get
  to the more expensive TLS handshake.
- **Server-side code** — no changes to `src/hermes_orch/`. v0.7.3 is
  client-side only. The server is the same as after v0.7.2 (HTTPS on,
  B12 + R7-C + v0.7.1 + v0.7.2 in the build).

**Files changed in v0.7.3:**

| File | Before | After | Delta |
|---|---|---|---|
| `installer/bootstrapper/install-orch-client.ps1` | 42112 bytes (20 functions, 14 error cases) | 47299 bytes (20 functions, 14 error cases; 4172 tokens) | +5187 bytes, +372 tokens, +1 function logic, 0 new error cases |
| `docs/runbooks/orch-client-install-runbook.md` | v0.4.1 | v0.4.2 (added §0.4 changelog rows 17-19) | +3 rows in §0 changelog |
| `docs/proposals/orch-client-build-impl-plan-v0.7.md` | v0.7 + cert-pinning + v0.7.1 + v0.7.2 | v0.7 + cert-pinning + v0.7.1 + v0.7.2 + v0.7.3 (this section) | +§0.quint section, +§11 changelog row, +§9 Q acceptance criteria |

**Acceptance criteria for v0.7.3 (all met, verified):**

1. PowerShell parser: 0 errors (verified: 4172 tokens parsed clean)
2. UTF-8 BOM preserved: `EF BB BF` (verified)
3. Function count: 20 (unchanged; v0.7.3 modified `Wait-ForEnrollment` in place)
4. `Wait-ForEnrollment` now uses `HttpClient` + cert validation callback
   (verified: function body has `HttpClientHandler`, `ServerCertificateCustomValidationCallback`,
   `SendAsync`, `GetAwaiter().GetResult()`)
5. Cert fingerprint SHA-256 compare: `cert.Export(X509ContentType::Cert)` + `[SHA256]::ComputeHash`
   (verified: lines 542-548 of the function)
6. TLS 1.2 forced: `SecurityProtocol = Tls12` (verified: line 565)
7. `CERT_MISMATCH` error message updated to cover TLS handshake failure
   (verified: line 620+)
8. **NOT YET VM-TESTED**: actual end-to-end cert fingerprint pinning
   against the live orch on Windows A. The PowerShell parser check
   is the static gate; the VM test gate is a follow-up (same gating
   pattern as v0.7.1 / v0.7.2 bootstrapper).

**Cross-language contract note (per the 2026-08-13 cross-language
compat test pattern):**

- The **HMAC signing** contract (Python `tests/helpers/hmac_v07.py`
  vs PowerShell `Wait-ForEnrollment` canonical input) was byte-equal
  on 2026-08-13; the golden test is in
  `tests/golden/hmac_v07_golden.json` on the server branch.
- The **cert fingerprint SHA-256** contract (the cert pinning
  comparison) is purely client-side; the server doesn't see it. The
  bootstrapper's PowerShell implementation (v0.7.3) is the only
  implementation. A future Python implementation (e.g. for the
  orch-client's own cert pinning) should follow the same
  `cert.Export(DER) + SHA256 + lowercase hex + compare` pattern.
- The compat test files in `C:\Users\stanley\AppData\Local\Temp\`
  (`compat_python.py`, `compat_powershell.ps1`, `compat_diff.py`)
  cover the HMAC contract; cert pinning has no cross-language
  contract to test today.
...
**Q (v0.7.3 added — real TLS cert fingerprint pinning)**:
- Bootstrapper `install-orch-client.ps1::Wait-ForEnrollment` uses `[System.Net.Http.HttpClient]` with a custom `HttpClientHandler.ServerCertificateCustomValidationCallback` that:
  - Computes SHA-256 of `cert.Export([X509ContentType]::Cert)` (DER bytes — NOT `GetCertHash()`, which is SHA-1)
  - Compares lowercase hex to user-pasted `$CertFingerprint` (64-char hex)
  - Returns `$true` only on match; fail closed on any error or mismatch
- TLS 1.2 forced via `[Net.ServicePointManager]::SecurityProtocol = Tls12` (PS 5.1 default is SSL3 / TLS 1.0, which the orch's gen-cert cert rejects)
- New `-CertFingerprint` parameter on `Wait-ForEnrollment`, passed from the call site (which has `$CertFingerprint` from the prompt)
- `CERT_MISMATCH` plain-English error message updated to cover both cert mismatch AND TLS handshake failure
- PowerShell parser: 0 errors; 4172 tokens; UTF-8 BOM preserved; 20 functions (unchanged from v0.7.2)
- Replaces the v0.7.1 / v0.7.2 `Invoke-WebRequest` which (a) has a PS 5.1 .NET bug breaking TLS 1.2, and (b) trusted default OS cert validation, which would fail outright for self-signed orch certs
- `Test-CertFingerprint` (pre-flight) is kept as an early-fail format check (still regex `^[0-9a-f]{64}$`), now backed by a real TLS check at handshake time
...
| Bootstrapper (v0.7.3) | (n/a) | `install-orch-client.ps1::Wait-ForEnrollment` rewritten to use `[System.Net.Http.HttpClient]` + `HttpClientHandler.ServerCertificateCustomValidationCallback` that computes SHA-256 of cert DER bytes and compares to the user-pinned `$CertFingerprint`. Force TLS 1.2 via `SecurityProtocol = Tls12`. New `-CertFingerprint` parameter at call site. Improved `CERT_MISMATCH` error message covers both cert mismatch and TLS handshake failure. Explicit `Dispose` of `HttpClient` / `HttpClientHandler` / `HttpRequestMessage`. Closes the gap where v0.7.1 / v0.7.2's format check was a hopeful lie — the bootstrapper's enrollment poll couldn't actually complete end-to-end against a self-signed orch |
